...
contract('UniversalResolver', function () {
  ...
  describe('encodeName()', () => {
    it('should encode a name', async () => {
      const result = await testNameEncoder.encodeName('foo.jfin')
      expect(result['0']).to.equal(dns.hexEncodeName('foo.jfin'))
      expect(result['1']).to.equal(namehash('foo.jfin'))
    })
    ...
    it('should encode a long name', async () => {
      const result = await testNameEncoder.encodeName(
        'something.else.test.jfin',
      )
      expect(result['0']).to.equal(
        dns.hexEncodeName('something.else.test.jfin'),
      )
      expect(result['1']).to.equal(namehash('something.else.test.jfin'))
    })
  })
})
